...
class Preprocessing:
    ...
    def subsample_prob(self, word, t=1e-3):
...
class PrepCbow(Preprocessing):
    ...
    def __init__(self, 
        ...
        super().__init__(
            vocab_size, unigram_table_size, 
            window_size, alpha, subsampling_threshold, 
            neg_samples_sum, tokenizer
        )
    ...
    def __call__(self, batch):
        ...
        for tweet in batch:
            ...
            for w in tweet:
                if w in self.vocab.word2idx and random.random() < self.subsample_prob(w):
            ...
            for idx, target_word in enumerate(tokens_subsample):
                ...
                contexts_words_idxs = list(map(lambda word: self.vocab.word2idx[word], contexts_words)) 
                contexts_words_idxs += [0 for _ in range(self.neg_samples_sum - len(contexts_words_idxs))]
        ...
        return torch.LongTensor(targets), torch.LongTensor(contexts), torch.LongTensor(neg_samples), len(batch)
    ...
    def __call__(self, batch):
        ...
        for tweet in batch:
            ...
            for index, target_word in enumerate(tokens_subsample):    
                ...
                neg_sample = [
                    self.unigram_table.samples(self.neg_samples_sum) for _ in range(len(contexts_words))
                ]

                targets += ([self.vocab.word2idx[target_word]] * len(contexts_words))
                contexts += list(map(lambda word: self.vocab.word2idx[word], contexts_words))
